...
def annotate_record(record: dict) -> dict:
    ...
    leak = detect_leakage(output, gold)
    ...
    compliance = check_step_compliance(output, gold)
...
def main() -> None:
    ...
    if args.models:
        model_dirs = [path for path in model_dirs if path.name in args.models]
    ...
    for model_dir in model_dirs:
        ...
        system_files = [
            ...
            if not path.stem.endswith("_evaluated")
        ]
        ...
        for system_file in system_files:
            ...
            logger.info("Evaluating %s/%s (%s)", model_id, system_id, system_file.name)
            records = load_results(system_file)
            if not records:
                logger.warning("  No records found, skipping.")
                continue

            annotated = [annotate_record(record) for record in records]

            annotated_path = model_dir / f"{system_id}_evaluated.jsonl"
            with open(annotated_path, "w", encoding="utf-8") as f:
                for record in annotated:
                    f.write(json.dumps(record, ensure_ascii=False) + "\n")

            all_aggs[f"{model_id}/{system_id}"] = aggregate(annotated)
