...
-----
    ...
    python run_experiment.py --systems B0 B1       # run only baselines
    python run_experiment.py --models general       # one model only
    python run_experiment.py --sample-size 50       # quick test run
    python run_experiment.py --math-dataset /path/to/math.json \
        --attack-prompts /path/to/dataset_b.json
    python run_experiment.py --use-local-hf         # load HF model locally
"""
...
DEFAULT_MATH_DATASET_PATH = PROJECT_ROOT / "Data" / "math.json"
DEFAULT_ATTACK_PROMPTS_PATH = PROJECT_ROOT / "Data" / "dataset_b.json"
...
def main():
    ...
    parser.add_argument(
        ...
        help="Path to the math dataset JSON (default: Data/math.json).",
    )
    parser.add_argument(
    ...
    math_dataset_path = args.math_dataset.expanduser()
    attack_prompts_path = args.attack_prompts.expanduser()
    for label, path in (
